...
class GameOfLife:
    def __init__(self, root, width=60, height=44, cell_size=13):
        self.root = root
        ...
        # trail intensity per cell, decays each frame so recently-dead
        # cells leave a fading afterglow instead of just vanishing
        self.trail = [[0.0 for _ in range(width)] for _ in range(height)]
        ...
        self.canvas = tk.Canvas(root, width=width*cell_size, height=height*cell_size, bg="#06060f", highlightthickness=0)
        ...
        preset_menu = ttk.Combobox(controls, textvariable=self.preset_var, values=list(PRESETS.keys()), state="readonly", width=16)
    ...
    def cell_color(self, age):
        # newborn cells: bright cyan-green. older, long-lived cells drift
        # toward violet. mirrors the palette used in generate_gif.py
        a = min(age / 26.0, 1.0)
        r = int(40 + a * 170)
        g = int(235 - a * 120)
        b = int(210 + a * 45)
    ...
    def trail_color(self, intensity):
        # fading afterglow for recently-dead cells (phosphor effect)
        intensity = max(0.0, min(intensity, 1.0))
        r = int(6 + intensity * 40)
        g = int(8 + intensity * 90)
        b = int(16 + intensity * 90)
        return f"#{r:02x}{g:02x}{b:02x}"
    ...
    def draw_grid(self):
        ...
        for r in range(self.height):
            for c in range(self.width):
                ...
                elif self.trail[r][c] > 0.03:
                    color = self.trail_color(self.trail[r][c])
    ...
    def next_generation(self):
        ...
        new_trail = [[0.0 for _ in range(self.width)] for _ in range(self.height)]
        ...
        for r in range(self.height):
            for c in range(self.width):
                ...
                # decay trail, then re-ignite it if this cell was just alive
                decayed = self.trail[r][c] * 0.85
                new_trail[r][c] = max(decayed, 1.0 if alive else 0.0)
    ...
    def reset_grid(self):
        ...
        self.trail = [[0.0 for _ in range(self.width)] for _ in range(self.height)]
        self.generation = 0
        self.draw_grid()
